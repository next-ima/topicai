...
def new_topic(user_topic):
    ...
    existing_topic = topics.find_one({"keywords": user_topic})
    ...
    if existing_topic:
        ...
        topic = topics.insert_one({"keywords": user_topic})
    ...
    response = AI_client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user", "content": f"Can you tell me some news about the {user_topic}"}
        ]
    )
...
def seach_by_keyword(keyword):
    ...
    for topic in topics.find({"keywords": keyword}):
        print(f"Topic ID: {topic['_id']}, Keywords: {topic['keywords']}\n")

        for text in topic_updates.find({"topic_id": topic['_id']}):
            print(f"Update ID: {text['_id']}\n Summary: {text['summary']}\n Score: {text['score']}\n Update Time: {text['update_time']}\n")
